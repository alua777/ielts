...
export default function NewUserDashboard({ survey, onStartExam, onPractice, onSurvey }) {
  ...
  return (
    <div className="space-y-4">
      <section className="grid gap-4 rounded-lg border border-violet-100 bg-white p-5 shadow-sm lg:grid-cols-[1fr_320px]">
        <div>
          ...
          <div className="mt-5 flex flex-col gap-3 sm:flex-row">
            ...
            <button type="button" onClick={onPractice} className="min-h-11 rounded-lg border border-slate-200 bg-white px-5 text-[14px] font-bold text-violet-700">Practice weak sections</button>
          </div>
        </div>
      ...
      <section className="rounded-lg border border-dashed border-slate-300 bg-white p-5 text-center">
        ...
        <button type="button" onClick={onSurvey} className="mt-4 min-h-11 rounded-lg border border-slate-200 bg-white px-5 text-[14px] font-bold text-slate-700">Update survey</button>
      </section>
    </div>
  );
}
